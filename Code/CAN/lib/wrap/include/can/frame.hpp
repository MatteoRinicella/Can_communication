...
namespace can{
    ...
    struct Frame{
        ...
        static inline Frame make_iso_tp_sf_std(uint16_t can_id, std::initializer_list<uint8_t> uds) noexcept{ // uds sono i byte "veri" del servizio
            Frame f;
            f.id = static_cast<uint16_t>(can_id&0X7FF); //maschero gli 11 bit dell'id standard ( 0000 0111 1111 1111), è ancora una vota un cast ridondante
            uint8_t L = static_cast<uint8_t>(uds.size() > 7 ? 7 : uds.size()); 
            uint8_t buff[8] = {static_cast<uint8_t>(0X00 | L)}; // il primo byte è il PCI-> 0X0 -> single frame, nibble basso l -> numero di byte, gli altri elementi del buffer sono inizializzati a zero
            
            uint8_t i = 0;
            for(uint8_t b: uds){
                if(i >= L) break; // stop di sicurezza, esce dal ciclo quando ho già copirato L byte
                buff[i+1] = b;
                ++i;
            }
            f.set_data(buff, static_cast<uint8_t>(1+L)); //1+l Per escludere il PCI
            return f;
        }
    };
